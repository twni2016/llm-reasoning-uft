...
log_pth="../data/game24/train_logs/$model_suffix"
export WANDB_PROJECT="game24-$model_suffix"
    ...
    for lr in "${lrs[@]}"; do
        for ul_alpha in "${ul_alphas[@]}"; do
            for seed in "${seeds[@]}"; do
                echo "Running game24, $model_name, $dataset, lr $lr, ga $ga, alpha $ul_alpha, seed $seed"
                ...
                accelerate launch --config_file recipes/accelerate_configs/deepspeed_zero3.yaml fine_tune/ul.py \
                    "recipes/game24/$dataset.yaml" --model_name_or_path=$model_name \
