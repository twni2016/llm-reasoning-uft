...
class DataArguments:
    ...
    train_on_completion_only: bool = field(
        default=True,
        metadata={"help": ("Whether to use DataCollatorForCompletionOnlyLM.")},
    )
    chat_template: Optional[str] = field(
        default=None, metadata={"help": "The chat template to use."}
    )
    dataset_root: str = None
    positive_dataset_sources: Optional[List[str]] = field(
        default=None,
        metadata={"help": ("List of positive dataset's relative path.")},
    )
    negative_dataset_sources: Optional[List[str]] = field(
        default=None,
        metadata={"help": ("List of negative dataset's relative path.")},
    )
...
class ULConfig(SFTConfig):
    ul_alpha: float = field(
        ...
        metadata={
            "help": (
                "(1.0-alpha) * pos_loss(positive example) + alpha * neg_loss(negative example)."
            )
        },
    )
    ul_loss_type: str = field(
